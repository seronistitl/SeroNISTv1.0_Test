...
observeEvent(input$testData_selectClassCol, {
  if (input$testData_selectClassCol == "None"){
    testData_clearAddedClasses()
    
    rv$testData_selectClassCol = NULL
    rv$testData_addClass = NULL
    
    output$testData_uniqueClassesOutput <- NULL
    
  } else {
    # print(paste0("NEW - attempt to clear added classes test data"))
    # testData_clearAddedClasses()
...
testData_renderSelectclassDropdown <- function(){
  if (isTruthy(rv$testData_originalDF) && 
    ...
    ## 0315 added
      currClassCol = input$testData_selectClassCol
    # Compute unique classes in selected column
    newUniqueFromCol = unique(rv$testData_originalDF[, currClassCol])
...
observeEvent(input$testData_addClassBtn, {
  ...
  ## 041224 add class should clear table selections
  # If option B
  if (!isOptionA("test")){
    # Deselect rows in Raw data table
    rdtt_proxy %>% selectRows(NULL)
  } 
})
