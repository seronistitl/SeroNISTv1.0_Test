...
observeEvent(input$selectClassCol, {
  rv$currSelClassCol = input$selectClassCol
  # Clear added classes
  clearAddedClasses()
...
renderSelectclassDropdown <- function(){
  if (isTruthy(rv$originalDF) && isTruthy(input$selectClassCol) &&
      input$selectClassCol != "None"){
      # && input$selectClassCol != "classColumn"){
    
    # Compute unique classes in selected column
    # newUniqueFromCol = unique(rv$originalDF[, rv$selectClassCol])
    newUniqueFromCol = unique(rv$originalDF[, rv$currSelClassCol])
...
observeEvent(input$addClassBtn, {
  ...
  # Check if classcolumn is assigned column, if so change back here
  if (rv$currSelClassCol == "classColumn"){
    rv$currSelClassCol = input$selectClassCol
  }
  ...
  # If option B
  if (!isOptionA("train")){
    # Deselect rows in Raw data table
    rdt_proxy %>% selectRows(NULL)
  } 
})
...
observeEvent({sapply(rv$addClass$removeClass, function(x){
  ...
  # print(paste0("______________________________________________"))
  # print(paste0("rv$addClass:", rv$addClass))
  # print(paste0("=============================================="))
...
getNumClassRows <- function(){
  ...
  ## 032624 resolve "classcolumn doesn't exist" error
  if (!("classColumn" %in% colnames(tempdf)) && 
  ...
  for (ii in 1:length(sepClasses)){
    ...
    colSelected = rv$currSelClassCol
