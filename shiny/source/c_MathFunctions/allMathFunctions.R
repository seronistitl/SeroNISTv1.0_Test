...
computeClass <- function(outmat, df, x_var, y_var, z_var, allClasses){
  if (isTruthy(x_var) && isTruthy(y_var) && isTruthy(z_var)){
    # Extract antigen data 
    tdf = df[, c(x_var, y_var, z_var)]
  } else if (isTruthy(x_var) && isTruthy(y_var) && !isTruthy(z_var)){
  ...
  #   # The positive projected value will be classA
  #   classA = "classA"
  #   # The negative projected value will be classB
  #   classB = "classB"
  # } else {
    # Extract class data
  ...
  rdf = data.frame(cbind(df, allClassified, allPredClass), check.names = FALSE)
...
projectPoints <- function(outmat, tdf){
  ...
  if (isTruthy(nrow(tdf))){ ### 022924 REMOVED FOR optimPARALLEL
  # if (is.null(nrow(tdf))){
